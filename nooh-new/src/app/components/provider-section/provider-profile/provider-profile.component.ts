...
export class ProviderProfileComponent {
    ...
    if (this.profileForm.valid) {
      ...
      this.profileService.AddProvider(formData).subscribe(
        ...
        (error) => {
          ...
          // Check and log detailed error response from the server (if available)
          if (error.error) {
            console.error('Detailed error response from server:', error.error);
          }
        
          // Log the error message from the server (if available)
          console.error('Error message:', error.message);
        
          // Log the HTTP status code (500, 400, etc.)
          console.error('Error status:', error.status);
        
          // If an error message or stack trace exists, log it for additional details
          if (error.error && error.error.exception) {
            console.error('Exception details:', error.error.exception);
          }
        
          // Provide feedback to the user through a toastr error message
          this.toastr.error(
            'Failed to submit provider data. Please check the console for more details.',
            'Error!',
            {
              timeOut: 3000,
            }
  ...
  ngOnDestroy(): void {
    // Reset the providerId to -1 when the component is destroyed
    this.providerId = -1;
    this.profileService.setProviderId(-1);
  }
