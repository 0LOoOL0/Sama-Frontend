...
constructor(private productService:ProductService, private providerService:ProvidersService, private router: Router,
  ...
  async loadProducts() {
    try {
      ...
      this.provoderServicess.getProductByProviders(providerId)
        .subscribe({
          next: (response: any) => {
            console.log('Full Response:', response);
  
            // Map the response directly as an array of products
            this.products = response.data.map((product: any) => ({
              id: product.id,
              image: product.image_url || 'assets/default-image.png',
              new_price: product.new_price,
              old_price: product.old_price,
              percentage: product.discount || 0,
              pet_type: product.pet_type,
              provider_id: product.provider_id,
              service_image_url: product.image_url,
