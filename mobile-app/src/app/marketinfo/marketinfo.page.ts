...
@Component({
  selector: 'app-marketinfo',
  templateUrl: './marketinfo.page.html',
  styleUrls: ['./marketinfo.page.scss'],
})
export class MarketInfoPage implements OnInit {
  ...
  getPetDetails(id: number) {
    this.petInfoService.getPet(id).subscribe(
      async (response: any) => {
        ...
        if (!this.pet.description || this.pet.description.trim() === '') {
          this.pet.description = "No description available."; // Default message
          this.descriptionSnippet = "No description available.";
        } else {
          this.descriptionSnippet = this.pet.description.substring(0, 50); // Get snippet if description exists
        }
        this.calculateAge();
