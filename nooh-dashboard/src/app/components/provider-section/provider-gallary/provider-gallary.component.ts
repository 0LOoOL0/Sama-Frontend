...
extractFileName(url: string): string
{
  ...
  // This conditional statement is added because if user doesnt add filename and the formis updated/submitted
  // it will give error cause you cannot use .split() fucntion on a null
  if (!url) 
...
deleteFile(type: string): void {
  ...
  isBase64(url: string): boolean {
    ...
    // this conditonal statement was added because if the document was not added and is null
    // it will give error if used with the fucntion .startsWith()
    if (!url) {  // Check if the URL is null or an empty string
      return false;  // Return false if the URL is null or empty
  }
   ...
   async updateGallery(): Promise<void> {
    ...
    console.error('Error message:', error.message);    // Shows error message
    console.error('Error details:', error.error);      // Shows error details from the backend
    console.error('Stack trace:', error.error.stack_trace);  // Stack trace (if sent by backend)

    // Optional: Log additional info like the error status code, etc.
    console.error('Error status:', error.status);

    // Display the toast message for UI
    this.toastr.error('Failed to update gallery. Check the console for details.', 'Try Again', { timeOut: 3000 });                  
                }
            );
        } catch (error) {
      ...
      if (this.galleryForm.valid) {
        ...
        this.galleryService.uploadGallery(formData).subscribe(
          (response) => {
            ...
            this.toastr.success('Gallery record has been uploaded successfully', 'Good News|', {
                timeOut: 2000, 
            });     
            this.toastr.warning('Please don’t submit a new gallery. If you want to change the record, you can edit it from the All Providers list.', 'Try Again', {
                timeOut: 5000, 
            });          
            this.resetForm();
        },
        (error) => {
            ...
            console.error('Error during gallery upload:', error);
        
            // Show a user-friendly message
            this.toastr.error('Please submit correct information.', 'Try Again', {
                timeOut: 3000, 
            });
        
            // Optionally log more error details if they are available
            if (error.response) {
